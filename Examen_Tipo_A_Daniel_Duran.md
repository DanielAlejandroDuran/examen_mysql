...
empleados e
JOIN 
sucursal s ON e.sucursalid = s.id
JOIN 
municipio m ON s.municipioid = m.id
JOIN 
departamento d ON m.depid = d.id
JOIN 
pais p ON d.paisid = p.id;
```
...
clientes c
JOIN 
municipio m ON c.municipioid = m.id
JOIN 
departamento d ON m.depid = d.id
JOIN 
pais p ON d.paisid = p.id;
...
GROUP BY 
    s.id, s.nombre
HAVING 
    COUNT(e.empleado_id) >= 2
    AND AVG(e.salario) > 1800000;
```
